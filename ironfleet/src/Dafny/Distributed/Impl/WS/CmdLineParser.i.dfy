...
{
    ConstantsState([], StaticParams())
}

function ws_config_parsing(args:seq<seq<uint16>>) : ConstantsState
{
    if args != [] && |args[1..]| % 2 == 0 then 
        var (ok, endpoints) := parse_end_points(args[1..]);
        ConstantsState(endpoints,
                       StaticParams())
    else 
        ConstantsStateNull()
}

function ws_parse_id(ip:seq<uint16>, port:seq<uint16>) : EndPoint
{
    var (ok, ep) := parse_end_point(ip, port);
    ep
}

function ws_cmd_line_parsing(env:HostEnvironment) : (ConstantsState, EndPoint)
    ...
    reads if env != null then env.constants else null;
{
    ...
    if |args| < 2 then
        ...
        var config := ws_config_parsing(args[..|args|-2]);
        var me := ws_parse_id(penultimate_arg, final_arg);
        (config, me)
}
...
method parse_cmd_line(ghost env:HostEnvironment) returns (ok:bool, config:ConstantsState, my_index:uint64)
    ...
    ensures var (config', my_ep') := ws_cmd_line_parsing(env);
            ok ==> config == config' && config.hostIds[my_index] == my_ep';
{
    ...
    config := ConstantsState(hosts, StaticParams());

    ghost var ghost_tuple := ws_cmd_line_parsing(env);
