...
module ReductionModule
{
    ...
    predicate EntriesReducibleUsingPivot(entries:seq<Entry>)
    {
       ...
       && var pivot_index := last(entries).pivot_index;
          0 < pivot_index < last(entries).group_len == |entries|
       && (forall i :: 0 <= i < pivot_index ==> EntryIsRightMover(entries[i]))
        ...
        decreases |entries|, 0;
    {
        ...
        && var entries_restricted := RestrictEntriesToLevel(entries, entries[0].begin_group_level);
           ActorTraceValid(middle(entries), min_level, entries[0].begin_group_level)
        && EntriesReducibleUsingPivot(entries_restricted)
        && EntriesReducibleToEntry(entries_restricted, last(entries).reduced_entry)
    }
    ...
    lemma lemma_IfActorTraceValidForLevelsThenReduceEntryWithinRange(
        trace:Trace,
        min_level:int,
        max_level:int,
        i:int
        ...
        decreases |trace|, 1;
        ensures  GetEntryLevel(trace[i].reduced_entry) <= max_level;
    {
        if i == 0 {
            var group_len :|    0 < group_len <= |trace|
                          && EntryGroupValidForLevels(trace[..group_len], min_level, max_level)
                          && ActorTraceValid(trace[group_len..], min_level, max_level);
            lemma_IfEntryGroupValidForLevelsThenReducedEntryLevelWithinRange(trace[..group_len], min_level, max_level, i);
        } else {
            if trace[0].EntryAction? && GetEntryLevel(trace[0]) == max_level && ActorTraceValid(trace[1..], min_level, max_level) {
                lemma_IfActorTraceValidForLevelsThenReduceEntryWithinRange(trace[1..], min_level, max_level, i-1);
            } else {
                var group_len :|    0 < group_len <= |trace|
                              && EntryGroupValidForLevels(trace[..group_len], min_level, max_level)
                              && ActorTraceValid(trace[group_len..], min_level, max_level);
                if i < group_len - 1 {
                    ...
                    assert GetEntryLevel(trace[i].reduced_entry) == max_level;
                } else {
                    lemma_IfActorTraceValidForLevelsThenReduceEntryWithinRange(trace[group_len..], min_level, max_level, i-group_len);
                }
            }
        }
    }

    lemma lemma_IfEntryGroupValidForLevelsThenReducedEntryLevelWithinRange(
        trace:Trace,
        min_level:int,
        max_level:int,
        i:int
        ...
        ensures  GetEntryLevel(trace[i].reduced_entry) < max_level;
    {
        if i == 0 {
            ...
            assert |trace'| == |trace| - 2;
            assert i <= |trace| - 2;
            assert ActorTraceValid(trace', min_level, trace[0].begin_group_level);
            lemma_IfActorTraceValidForLevelsThenReduceEntryWithinRange(trace', min_level, trace[0].begin_group_level, i-1);
        }
    }
    ...
    {
        forall j | 0 <= j < |trace| - 1 && trace[j].EntryEndGroup?
            ensures GetEntryLevel(trace[j].reduced_entry) < max_level;
        {
            lemma_IfEntryGroupValidForLevelsThenReducedEntryLevelWithinRange(trace, min_level, max_level, j);
        } 
...
//        lemma_ElementFromSequenceSlice(trace, trace', 1, group_len - 1, i);
    ...
    function RestrictEntriesToLevelFirstStep(entry:Entry, level:int) : Trace
    {
        if GetEntryLevel(entry) == level then
            [entry] 
        else if entry.EntryEndGroup? && GetEntryLevel(entry.reduced_entry) == level then
            [entry.reduced_entry] 
        else 
            []
    }
    ...
    lemma lemma_RestrictEntriesToLevelAdds(e1:seq<Entry>, e2:seq<Entry>, level:int)
        ensures RestrictEntriesToLevel(e1, level) + RestrictEntriesToLevel(e2, level) == RestrictEntriesToLevel(e1+e2, level);
    {
        reveal_RestrictEntriesToLevel();
...
*/
    ...
    predicate DistributedSystemBehaviorRefinesSpecBehaviorWithConsecutiveNonPivotsAsStutters(
        db:seq<DistributedSystemState>,
        sb:seq<SpecState>,
        begin_entry_pos:int,
        end_entry_pos:int,
    ...
    lemma lemma_PerformOneReductionStep(
        ...
        ensures  DistributedSystemBehaviorRefinesSpec(db')
                 ==> exists sb :: DistributedSystemBehaviorRefinesSpecBehaviorWithConsecutiveNonPivotsAsStutters(db, sb, begin_entry_pos, end_entry_pos, pivot_index);
        ensures  trace' == trace[..begin_entry_pos] + [last(group).reduced_entry] + trace[end_entry_pos+1 ..];
    {
        ...
        forall i {:trigger DistributedSystemNextEntryAction(tiny_db[i], tiny_db[i+1], group[i])} | 0 <= i < |tiny_db|-1
            ensures DistributedSystemNextEntryAction(tiny_db[i], tiny_db[i+1], group[i]);
        {
