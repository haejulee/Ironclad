...
module ReductionBasicModule
{
    ...
    lemma lemma_IfEntriesReducibleAndOneIsntRightMoverThenRestAreLeftMovers(entries:seq<ExtendedEntry>, pivot_index:int, i:int, j:int)
        requires 0 <= i < j < |entries|;
        requires 0 <= pivot_index <= |entries|;
        requires forall k :: 0 <= k < pivot_index ==> ExtendedEntryIsRightMover(entries[k]);
        requires forall k :: pivot_index < k < |entries| ==> ExtendedEntryIsLeftMover(entries[k]);
        requires !ExtendedEntryIsRightMover(entries[i]);
        ensures  ExtendedEntryIsLeftMover(entries[j]);
        decreases j;
    {
    ...
    lemma lemma_IfEntriesReducibleThenSuffixIs(entries:seq<ExtendedEntry>, entries':seq<ExtendedEntry>, pivot_index:int) returns (new_pivot_index:int)
        requires |entries| > 0;
        requires entries' == entries[1..];
        requires 0 <= pivot_index <= |entries|;
        requires forall i :: 0 <= i < pivot_index ==> ExtendedEntryIsRightMover(entries[i]);
        requires forall i :: pivot_index < i < |entries| ==> ExtendedEntryIsLeftMover(entries[i]);
        ensures  0 <= new_pivot_index <= |entries'|;
        ensures  forall i :: 0 <= i < new_pivot_index ==> ExtendedEntryIsRightMover(entries'[i]);
        ensures  forall i :: new_pivot_index < i < |entries'| ==> ExtendedEntryIsLeftMover(entries'[i]);
    {
        if |entries'| == 0 {
            new_pivot_index := 0;
            return;
